...
class SalesDashboardController extends Controller
{
    ...
    private function paginateArray($items, $perPage = 25, $pageName = 'page')
    {
        $items = collect($items);
    ...
    public function salesReturnIndex(Request $request)
    {
        ...
        $distributionChannels = $sdndwh->table('Fact_Sales_Business')
            ->select('Dist Channel AS DistributionChannel')
            ->distinct()
        ...
        // $customerGroup = $sdndwh->table('Fact_Sales_Business')
        //     ->select('Customer Group AS CustomerGroup')
        //     ->distinct()
        //     ->orderBy('CustomerGroup', 'ASC')
        //     ->get();

        $principals = $sdndwh->table('Fact_Sales_Business')
            ->select('Div Code AS PrincipalCode', 'DIV Desc AS PrincipalName')
            ->distinct()
        ...
        // 🔹 TEMP DEBUG (keep this if you're testing)
